...
$users_for_initiale = $con->getData("SELECT id, CONCAT_WS(' ',fname, lname) description FROM users WHERE id IN ($users_ids_initial)");
...
$users_for_signature = $con->getData("SELECT id, CONCAT_WS(' ',fname, lname) description FROM users WHERE id IN ($users_ids_signature)");
